...
int kgsl_pwrctrl_init(struct kgsl_device *device)
{
	 ...
	 * Optimal -> opt_pwrlevel -> 320MHz
	 * Medium  -> med_pwrlevel -> 200MHz
	 * Minimum -> min_pwrlevel -> 100MHz
	 */
	pwr->max_pwrlevel = 0; /* 533MHz */
	pwr->opt_pwrlevel = pdata->num_levels - 4; /* 320MHz */
	pwr->med_pwrlevel = pdata->num_levels - 3; /* 200MHz */
	pwr->min_pwrlevel = pdata->num_levels - 2; /* 100MHz */
	pwr->thermal_pwrlevel = 0;

	pwr->active_pwrlevel = pwr->med_pwrlevel; /* 200MHz */
	pwr->default_pwrlevel = pwr->min_pwrlevel; /* 100MHz */
	pwr->init_pwrlevel = pwr->opt_pwrlevel; /* 320MHz */
